...
export async function getGlobalData({
  ...
  try {
    if (!pageId) {
      console.error('Notion page ID is missing')
      return data
    }
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  const menuPages = collectionData.filter(
    post =>
      post.status === 'Published' &&
      (post?.type === 'Menu' || post?.type === 'SubMenu' || post?.type === 'ChildMenu')
  )
  const menus = []
  if (menuPages && menuPages.length > 0) {
    menuPages.forEach(e => {
      ...
      } else if (e.type === 'ChildMenu') {
        // 處理三級菜單 - 查找其父級 SubMenu
        const parentMenu = menus[menus.length - 1]
        if (parentMenu && parentMenu.subMenus) {
          // 尋找最後一個添加的 SubMenu 作為父級
          const parentSubMenu = parentMenu.subMenus[parentMenu.subMenus.length - 1]
          if (parentSubMenu) {
            if (parentSubMenu.childMenus) {
              parentSubMenu.childMenus.push(e)
            } else {
              parentSubMenu.childMenus = [e]
            }
          }
        }
      }
 ...
 * @param {string} options.locale - 語言
 * @param {Array} options.excludePages - 要排除的頁面
 * @returns {Promise<Array>} - 所有頁面數組
 */
export async function getAllPagesCache({ locale, excludePages = [] }) {
  try {
    const allPages = await getAllPages({ locale, from: 'getAllPagesCache' })
    
    // 如果提供了排除頁面列表，進行過濾
    if (excludePages && excludePages.length > 0) {
      return allPages.filter(post => !excludePages.includes(post.slug))
    }
    
    return allPages
  } catch (error) {
    console.error('獲取所有頁面緩存失敗', error)
    return []
  }
}
...
/**
 * 獲取站點配置值
 * @param {Object} param0 - 參數對象
 * @param {Object} param0.notionConfig - Notion 配置
 * @returns {Function} - 獲取配置的函數
 */
export function getSiteConfigValue({ notionConfig }) {
  return function(key, defaultValue) {
    // 嘗試從 notionConfig 獲取值
    if (notionConfig && notionConfig[key] !== undefined) {
      return notionConfig[key]
    }
