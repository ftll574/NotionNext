...
function getRawNotion() {
  if (!globalStore.notion) {
    globalStore.notion = new NotionLibrary({
      ...
      // notion-client 7.x uses ofetch, so the previous kyOptions object was
      // ignored. Configure ofetch directly; Cloudflare now rejects its default
      // `User-Agent: node` with HTTP 403.
      ofetchOptions: {
        mode: 'cors',
        headers: {
          // Cloudflare rejects ofetch's default `User-Agent: node`. Identify
          // this public-site client explicitly instead of impersonating a browser.
          'User-Agent': 'NotionNext/4.9 (+https://xin-wei.com)'
        }
      }
    })
  }
