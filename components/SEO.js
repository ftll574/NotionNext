...
const SEO = props => {
  ...
  const generateOrganizationSchema = () => {
    const organizationSchema = {
      ...
      'description': '鑫葳貿易專注於高品質塑膠原料的供應與服務，為電子、汽車、家電等行業提供可靠的原材料支持',
      'address': {
        '@type': 'PostalAddress',
        'addressLocality': '新北市',
        'addressRegion': '林口區',
  ...
  const generateProductSchema = () => {
    if (!meta || meta.type !== 'Product' || router.route !== '/products') return null
    
    const productSchema = {
      '@context': 'https://schema.org',
      '@type': 'Product',
      'name': '鑫葳塑膠原料',
      'description': '高品質塑膠原料，包括PP聚丙烯、PE聚乙烯、PS聚苯乙烯等通用塑料和工程塑料',
      'brand': {
        '@type': 'Brand',
        'name': AUTHOR
      },
  ...
  return (
    <Head>
      ...
      {mainStructuredData.map(schema => (
        <script key={schema.id} type="application/ld+json" dangerouslySetInnerHTML={{ __html: schema.content }} />
      ))}
  ...
  switch (router.route) {
    ...
    default:
      return {
        ...
        description: post?.summary || `鑫葳貿易有限公司 - 專業塑膠原料供應商，提供PP聚丙烯、PE聚乙烯等多種高品質塑膠原料，為各行業提供全面技術支援。`,
